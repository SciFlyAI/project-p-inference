...
def get_tiles(image, size_crop, log=log, verbose=False):
    ...
    assert (
        isinstance(size_crop, (int, float))
        or isinstance(size_crop, (dict, list, set, tuple))
        and len(size_crop) == 2
    ), (
    ...
    tiles = np.array([None] * num_patches_x * num_patches_y).reshape(
    ...
    for i in range(num_patches_x):
        ...
        for j in range(num_patches_y):
            ...
            shift_y = (
                (num_patches_y - j) - (height - offset_y) / size_crop[1]
...
def draw_detections(
    ...
    assert shape in [
        'box',
        'dot',
    ], "Detection shape must be either 'box' or 'dot'!"
    if shape == 'box':
    ...
    for i, coords in enumerate(coords_all):
        ...
        if shape == 'box':
            frame = cv.rectangle(
                frame,
                coords[:2].round().astype('int'),
                coords[2:4].round().astype('int'),
                color=color,
                thickness=2,
            )
        else:
    ...
    cv.putText(
        frame,
        text,
        tuple(map(lambda x: round(x + 1), text_origin)),
        font_face,
        font_scale,
        (0, 0, 0),
        font_thickness,
    ...
    cv.putText(
        frame,
        text,
        tuple(map(lambda x: round(x), text_origin)),
        font_face,
        font_scale,
        (255, 255, 255),
        font_thickness,
...
def get_percentile(detections: np.ndarray, percentile: int = 99):
    ...
    if not len(detections):
        return 0.0
    detections_count, bins = np.histogram(
        detections, bins=detections.max().astype(int) + 1
    )
